...
int main()
{
    ...
    const char *fifo_path = "./italy_pipe";
    const char *watchdog_fifo = "watchdog_pipe";

    int wd_fd = open(watchdog_fifo, O_RDONLY);
    if (wd_fd == -1)
    {
    ...
    while (running)
    {
        ...
        mvwprintw(win, 1, (win_width - 10) / 2, "Italy Rome");
        mvwprintw(win, 2, (win_width - 10) / 2, "##########");
        ...
        if (difftime(current_time, last_time_sent) > TIMEOUT)
        {
            kill(watch_dog_pid, SIGUSR2);
