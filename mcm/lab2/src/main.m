...
% Calculate the transformation from joint 3 to joint 5 (^5T_3)
bT3 =  geometricModel.getTransformWrtBase(3); % Transformation from base to joint 3
bT5 =  geometricModel.getTransformWrtBase(5); % Transformation from base to joint 5
T5_3 = bT3 \ bT5; % Compute ^5T_3 using the inverse of bT3
T5_3= inv(T5_3);
disp('5T3');
disp(T5_3);
disp('p of 5T3');
...
qi = q;
qf = [pi/4, -pi/4, 0, -pi/4, 0, 0.15, pi/4];;
...
for i = 1:samples
    ...
    bri= zeros(3,length(geometricModel.jointType));
    q = qSteps(i,1:length(geometricModel.jointType))';
    ...
    if (rem(i,0.1) == 0) % only every 0.1 sec
        ...
        for j = 1:length(geometricModel.jointType)
            bTi(:,:,j) = geometricModel.getTransformWrtBase(j); 
        end

        bri(:, 1) = [0; 0; 0]; % Base position at the origin

        for j = 1:length(geometricModel.jointType)
            bri(:, j+1) = bTi(1:3, 4, j); % Assign the position vector (x, y, z)
        end
